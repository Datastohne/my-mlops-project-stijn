...
* Feature computation modules under `my_mlops_project_stijn/feature_engineering` folder. 
...
* Python unit tests for feature computation modules in `my_mlops_project_stijn/tests/feature_engineering` folder.
* Feature engineering notebook, `my_mlops_project_stijn/feature_engineering/notebooks/GenerateAndWriteFeatures.py`, that reads input dataframes, dynamically loads feature computation modules, executes their `compute_features_fn` method and writes the outputs to a Feature Store table (creating it if missing).
...
* An automated integration test is provided (in `.github/workflows/my-mlops-project-stijn-run-tests-fs.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
1. Implement your feature module, address TODOs in `my_mlops_project_stijn/feature_engineering/features` and create unit test in `my_mlops_project_stijn/tests/feature_engineering`
2. Update `my_mlops_project_stijn/databricks-resources/feature-engineering-workflow-resource.yml`. Fill in notebook parameters for `write_feature_table_job`.
3. Update training data path in `my_mlops_project_stijn/databricks-resources/model-workflow-resource.yml`.

We expect most of the development to take place in the `my_mlops_project_stijn/feature_engineering` folder.

## Iterating on ML code

### Deploy ML code and resources to dev workspace using Bundles

Refer to [Local development and dev workspace](../my_mlops_project_stijn/databricks-resources/README.md#local-development-and-dev-workspace)
...
* Run `databricks configure --profile my-mlops-project-stijn-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile my-mlops-project-stijn-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)

#### Running code on Databricks
You can iterate on ML code by running the provided `my_mlops_project_stijn/feature_engineering/notebooks/GenerateAndWriteFeatures.py` notebook on Databricks using
...
#### Prerequisites
* Python 3.8+
* Install feature engineering code and test dependencies via `pip install -I -r my_mlops_project_stijn/requirements.txt -r test-requirements.txt` from project root directory.

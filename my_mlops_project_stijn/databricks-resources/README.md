...
The databricks CLI bundles top level is defined by file `my_mlops_project_stijn/bundle.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`my_mlops_project_stijn/databricks-resources/model-workflow-resource.yml`)
 - batch inference workflow (`my_mlops_project_stijn/databricks-resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`my_mlops_project_stijn/databricks-resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`my_mlops_project_stijn/databricks-resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`my_mlops_project_stijn/databricks-resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` environment is used by ML engineers to deploy ML resources to development workspace with `dev` environment configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-my-mlops-project-stijn-model     | /dev-my-mlops-project-stijn-experiment     |
| staging     | The `staging` environment is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` environment config.                                                             | staging workspace    | staging-my-mlops-project-stijn-model | /staging-my-mlops-project-stijn-experiment |
| prod        | The `prod` environment is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` environment config.                                                                      | prod workspace       | prod-my-mlops-project-stijn-model    | /prod-my-mlops-project-stijn-experiment    |
| test        | The `test` environment is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` environment. | staging workspace    | test-my-mlops-project-stijn-model    | /test-my-mlops-project-stijn-experiment    |
...
2. Complete the `TODO` in `my_mlops_project_stijn/bundle.yml` to add the dev workspace URI under `environments.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `my_mlops_project_stijn/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `my_mlops_project_stijn/databricks-resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.environment}-my-mlops-project-stijn-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.environment}-my-mlops-project-stijn-batch-inference-job`
that runs the notebook under `my_mlops_project_stijn/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.environment}-my-mlops-project-stijn-batch-inference-job`. 
...
`test-my-mlops-project-stijn-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-my-mlops-project-stijn-batch-inference-job`.
